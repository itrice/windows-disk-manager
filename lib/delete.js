...
/* Node-12-safe fs helpers (fs.promises.rm/cp only exist on Node >= 14.14 / 16.7) */
async function removePath(p) {
  let st;
  try { st = await fs.promises.lstat(p); } catch (e) { if (e.code === 'ENOENT') return; throw e; }
  if (st.isDirectory()) await fs.promises.rmdir(p, { recursive: true });
...
async function restoreFromLocalTrash(config, itemPath) {
  ...
  try {
    ...
    if (e.code === 'EXDEV') {
      await copyRecursive(itemPath, origPath);
      await removePath(itemPath);
    } else throw e;
  }
  await removePath(metaPath).catch(() => {});
