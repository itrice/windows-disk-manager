...
class DiskScanner {
  ...
  async scan(root, opts = {}) {
    ...
    const maxDepth = opts.maxDepth != null ? opts.maxDepth : (this.config.maxDepth != null ? this.config.maxDepth : 40);
    const concurrency = Math.max(1, opts.concurrency != null ? opts.concurrency : (this.config.concurrency != null ? this.config.concurrency : 12));
...
function buildDerived(data) {
  ...
  const now = Date.now();
  const oldCutoff = now - (config.oldFileDays != null ? config.oldFileDays : 180) * 86400000;
  ...
  const report = {
    meta: {
      ...
      oldFileDays: config.oldFileDays != null ? config.oldFileDays : 180,
    },
    totals: {
      files: files.length,
